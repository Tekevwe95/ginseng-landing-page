...
function addIngredientsSection() {
  ...
  const style = document.createElement("style"); style.id = "ingredients-section-styles"; style.textContent = `.ingredients-section{background:var(--cream);padding:90px 0;overflow:hidden}.ingredients-intro{max-width:720px;margin:0 auto 42px;text-align:center}.ingredients-intro .eyebrow{margin-bottom:12px}.ingredients-intro h2{font-size:clamp(2.2rem,4vw,3.4rem)}.ingredients-intro p:last-child{color:var(--muted);margin-top:16px}.ingredient-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;max-width:900px;margin:0 auto}.ingredient-card{display:flex;align-items:center;gap:17px;background:#fff;border:1px solid #efc68e;border-radius:18px;padding:16px 18px;min-height:92px;transition:transform .22s,box-shadow .22s,border-color .22s}.ingredient-card:hover{transform:translateY(-4px);border-color:#dca55c;box-shadow:0 14px 35px rgba(18,59,39,.10)}.ingredient-card:last-child{grid-column:1/-1;max-width:442px;width:100%;justify-self:center}.ingredient-icon{width:56px;height:56px;flex:0 0 56px;border-radius:50%;display:grid;place-items:center;background:#ffebc9;font-size:28px;line-height:1}.ingredient-card h3{font-family:"Playfair Display",serif;font-size:1.08rem;margin-bottom:2px;color:var(--text)}.ingredient-card p{font-size:.82rem;color:#8a8178;line-height:1.45}@media(max-width:800px){.ingredients-section{padding:70px 0}.ingredient-grid{grid-template-columns:1fr;gap:12px}.ingredient-card:last-child{grid-column:auto;max-width:none}.ingredient-card{min-height:82px;padding:13px 14px;border-radius:15px}.ingredient-icon{width:50px;height:50px;flex-basis:50px;font-size:25px}.ingredient-card h3{font-size:1rem}.ingredient-card p{font-size:.77rem}}`; document.head.appendChild(style);
  const section=document.createElement("section"); section.id="ingredients"; section.className="ingredients-section"; section.innerHTML=`<div class="container"><div class="ingredients-intro"><p class="eyebrow">THE FIVE TREASURES</p><h2>What's inside your cup?</h2><p>Your daily cup, made from nature.<br>Five whole herbs. Nothing unnecessary.</p></div><div class="ingredient-grid"><article class="ingredient-card"><div class="ingredient-icon">🌿</div><div><h3>Ginseng Root</h3><p>Core energy restorer — the king of herbs</p></div></article><article class="ingredient-card"><div class="ingredient-icon">🔴</div><div><h3>Wolfberry (Goji)</h3><p>Antioxidant powerhouse — kidney and liver tonic</p></div></article><article class="ingredient-card"><div class="ingredient-icon">🌹</div><div><h3>Red Dates</h3><p>Nourishes blood, strengthens immunity</p></div></article><article class="ingredient-card"><div class="ingredient-icon">🍃</div><div><h3>Mulberry Leaf</h3><p>Blood sugar balance and anti-aging</p></div></article><article class="ingredient-card"><div class="ingredient-icon">🌾</div><div><h3>Yellow Essence (Huang Jing)</h3><p>Lung health, respiratory support</p></div></article></div><div class="ingredient-note"><span>✓</span>10g of whole herbs per bag — brews 2–3 potent cups.</div></div>`; productSection.parentNode.insertBefore(section,productSection);
}

function addRoutineSection(){
  const productSection=document.querySelector(".product-section"); if(!productSection||document.getElementById("routine"))return;
  const style=document.createElement("style"); style.id="routine-section-styles"; style.textContent=`.routine-section{background:#fbf7ef;padding:88px 0;overflow:hidden}.routine-heading{max-width:820px;margin:0 auto 46px;text-align:center}.routine-heading .eyebrow{margin-bottom:12px}.routine-heading h2{font-size:clamp(2.25rem,4.5vw,3.7rem);margin-bottom:16px}.routine-heading p:last-child{color:var(--muted);font-size:1.05rem}.routine-steps{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;max-width:1100px;margin:0 auto}.routine-step{position:relative;background:#fff;border:1px solid #eadbbd;border-radius:22px;overflow:hidden;box-shadow:0 8px 28px rgba(18,59,39,.06)}.routine-number{position:absolute;z-index:2;top:-16px;left:50%;transform:translateX(-50%);width:58px;height:58px;border-radius:50%;display:grid;place-items:center;background:var(--green);color:#fff;font-weight:800;font-size:1.15rem;border:5px solid #fbf7ef}.routine-image{height:245px;width:100%;display:block;background-image:url('../assets/product.jpg');background-repeat:no-repeat;background-size:300% auto;background-color:#f4d8c5}.routine-image.drop{background-position:0% 72%}.routine-image.pour{background-position:50% 48%}.routine-image.steep{background-position:100% 42%}.routine-content{padding:22px 24px 26px}.routine-content h3{font-family:"Playfair Display",serif;font-size:1.7rem;color:var(--green-dark);margin-bottom:5px}.routine-content strong{display:block;font-size:1rem;margin-bottom:7px}.routine-content p{color:var(--muted);line-height:1.6;font-size:.92rem}@media(max-width:800px){.routine-section{padding:68px 0}.routine-heading{margin-bottom:38px}.routine-steps{grid-template-columns:1fr;gap:16px;max-width:520px}.routine-number{top:12px;left:22px;transform:none;border-color:#fff}.routine-image{height:220px;background-size:300% auto}.routine-content{padding:20px}.routine-content h3{font-size:1.5rem}}`; document.head.appendChild(style);
...
function addCustomerReviews(){
  ...
  reviewStyle.textContent=`.reviews-grid.real-reviews{grid-template-columns:repeat(2,minmax(0,1fr));max-width:980px;margin:0 auto}.customer-review{background:#fff;border:1px solid var(--border);border-radius:20px;padding:24px;overflow:hidden;box-shadow:0 10px 30px rgba(18,59,39,.07);transition:transform .25s ease,box-shadow .25s ease}.customer-review:hover{transform:translateY(-4px);box-shadow:0 18px 40px rgba(18,59,39,.12)}.customer-review .review-message{font-size:1.05rem;font-weight:700;line-height:1.6;color:var(--text)}.customer-review .review-response{margin-top:18px;padding-top:14px;border-top:1px solid var(--border);color:var(--muted);font-size:.9rem}.customer-review .review-label{display:inline-block;margin-bottom:12px;padding:5px 9px;border-radius:999px;background:#edf3e9;color:var(--green);font-size:.68rem;font-weight:900;letter-spacing:.8px}.customer-review .review-time{display:block;margin-top:7px;color:#8a918c;font-size:.72rem}@media(max-width:700px){.reviews-grid.real-reviews{grid-template-columns:1fr}}`;
